...
const CreateSystem = () => {
  ...
  const [form, setForm] = useState({ name: '', description: '', category: 'saas', visibility: 'private' })
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const { data } = await client.post('/systems', { ...form, tags: suggestion?.tags || [] })
      if (suggestion?.pages?.length) {
        await Promise.all(
          suggestion.pages.map((p, i) =>
            client.post(`/systems/${data.system.slug}/pages`, {
              name: p.name, description: p.description, icon: p.icon, order: i,
            }).catch(() => null)
          )
        )
      }
    ...
    <div className="min-h-screen p-8" style={{ backgroundColor: '#09090B' }}>
      <div className="max-w-2xl mx-auto flex flex-col gap-8">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm" style={{ color: '#52525B' }}>Define your product foundation.</p>
        </div>
        ...
        <div className="rounded-xl border p-5 flex flex-col gap-4"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium" style={{ color: '#A1A1AA' }}>AI Assistant — optional</span>
          </div>
          <div className="flex gap-2">
            <input type="text"
              placeholder='e.g. "A CRM for marketing agencies"'
              value={prompt}
              onChange={e => setPrompt(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleAISuggest()}
              className="flex-1 px-3 py-2.5 text-sm rounded-lg border"
              style={{ backgroundColor: '#09090B', borderColor: '#27272A', color: '#FAFAFA', outline: 'none' }}
          ...
          {suggestion?.pages?.length > 0 && (
            <div className="flex flex-col gap-2 pt-2 border-t" style={{ borderColor: '#1C1C1F' }}>
              ...
              <p className="text-xs" style={{ color: '#3F3F46' }}>These pages will be created automatically.</p>
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <Input label="System Name" type="text"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium uppercase tracking-wider" style={{ color: '#A1A1AA' }}>Category</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium uppercase tracking-wider" style={{ color: '#A1A1AA' }}>Visibility</label>
            <div className="flex gap-2">
              {[
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" loading={loading} size="md">Create System</Button>
            <Button type="button" variant="ghost" size="md" onClick={() => navigate('/dashboard')}>Cancel</Button>
          </div>
        </form>
      </div>
    </div>
